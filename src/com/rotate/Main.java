...
public class Main {
    ...
    private static void createEmptyBitMap(byte[] bytes, int height, int width, int offset) {
        ...
        int counter = offset; // start the counter from the pixel data offet
        for (int ty = 0; ty < height; ty++) {
            for (int tx = 0; tx < width; tx++) {
                int sx = height - ty - 1; // formula to calculate the source x  when rotated
                int sourceIndex = offset + 3 * (tx * width + sx); // formula to calculate the source index 
                copyBytes(bytes, emptyBitMapArray, counter, sourceIndex); // sequence to copy the 3 bytes from the source image
                counter += 3; // increment the counter by 3 to move to the next pixel
            }
        }
    ...
    private static void copyBytes(byte[] bytes, byte[] emptyBitMapArray, int counter, int sourceIndex) {
        emptyBitMapArray[counter] = bytes[sourceIndex]; // copy the first byte
        emptyBitMapArray[counter + 1] = bytes[sourceIndex + 1]; // copy the second byte
        emptyBitMapArray[counter + 2] = bytes[sourceIndex + 2]; // copy the third byte
    }
